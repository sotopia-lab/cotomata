...
def test_transformer_training_with_save():
    # Set device
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"Using device: {device}")
    ...
    # Test 1: Assert there are exactly 3 files
    assert len(files) == epochs, f"Expected 3 files, found {len(files)}"

    # Test 2: Assert one of the files is 'model.pt'
    assert f'model_epoch_{epochs}.pt' in files, f"'model_epoch_{epochs}.pt' not found in the folder"   
...
if __name__ == "__main__":
    ...
    test_transformer_training_with_save()